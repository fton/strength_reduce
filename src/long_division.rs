...
const fn divide_128_by_64_preshifted(numerator_hi: u64, numerator_lo: u64, divisor: u64) -> u64 {
	...
	// but the problem is, full_upper_numerator is a 96-bit number, meaning we would need to use u128 to 
	// do the division all at once, and the whole point of this is that we don't want to do 128 bit 
	// divison because it's slow so instead, we'll shift both the numerator and divisor right by 32, 
	// giving us a 64 bit / 32 bit division. This won't give us the exact quotient 
	// -- but it will be close.
    let full_upper_numerator = ((numerator_hi as u128) << 32) | numerator_mid;
    let mut quotient_hi = min_u64(numerator_hi / divisor_hi, U32_MAX);
    let mut product_hi = quotient_hi as u128 * divisor_full_128;

	// quotient_hi contains our guess at what the quotient is! the problem is that we got this by ignoring
	// the lower 32 bits of the divisor. when we account for that, the quotient might be slightly lower we
	// will know our quotient is too high if quotient * divisor > numerator. if it is, decrement until 
	// it's in range
    ...
    let mut quotient_lo = min_u64((remainder_hi as u64) / divisor_hi, U32_MAX);
...
// divides a 128-bit number by a 64-bit divisor, returning the quotient as a 64-bit number
// assumes that the divisor and numerator have both already been bit-shifted to maximize the number of 
// bits in divisor_hi divisor_hi should be the upper 32 bits, and divisor_lo should be the lower 32 bits
#[inline]
const fn divide_128_by_64_preshifted_reduced(
	numerator_hi: u64,
	numerator_lo: u64,
	divisor_hi: StrengthReducedU64,
	divisor_full: u64)
	-> u64
{
    let numerator_mid = (numerator_lo >> 32) as u128;
	...
	// -- but it will be close.
    let full_upper_numerator = ((numerator_hi as u128) << 32) | numerator_mid;
    let mut quotient_hi = min_u64(divisor_hi.divide(numerator_hi) , U32_MAX);
    let mut product_hi = quotient_hi as u128 * divisor_full_128;
    ...
    let mut quotient_lo = min_u64(divisor_hi.divide(full_upper_remainder as u64) , U32_MAX);
    let mut product_lo = quotient_lo as u128 * divisor_full_128;
...
pub(crate) const fn divide_128_max_by_64(divisor: u64) -> u128 {
	...
	let quotient_lo = if leading_zeros >= 32 {
		...
		let numerator_hi = if leading_zeros > 0 {
			(remainder_hi << leading_zeros) | (core::u64::MAX >> (64 - leading_zeros))
		} else {
			remainder_hi
		};
...
const fn divide_256_max_by_32(divisor: u32) -> (u128, u128) {
	let reduced_divisor = StrengthReducedU64::new(unsafe { NonZeroU64::new_unchecked(divisor as u64) });
	let quotient_chunks = long_division([core::u64::MAX; 4], &reduced_divisor, [0; 4]);
...
pub(crate) const fn divide_256_max_by_128(divisor: u128) -> (u128, u128) {
	...
	let divisor_slice_len = divisor_chunks.len() - empty_divisor_chunks;

	// We're gonna be doing a ton of u64/u64 divisions, so we're gonna eat our own dog food and set up a 
	// strength-reduced division instance the only actual **divisions* we'll be doing will be with the 
	// largest 32 bits of the full divisor, not the full divisor
	let divisor_hi = divisor_chunks[divisor_slice_len-1];
	let reduced_divisor_hi = StrengthReducedU64::new(unsafe {
		NonZeroU64::new_unchecked(divisor_hi >> 32)
	});

	// Build our numerator, represented by u32 chunks. at first it will be full of u32::MAX, but we will
	// iteratively take chunks out of it as we divide
	...
	// allocate the biggest-possible quotient, even if it might be smaller -- we just won't fill out the
	// biggest parts
	let num_quotient_chunks = 3 + empty_divisor_chunks;
	let mut quotient_chunks = [0; 4];
	let mut quotient_idx = num_quotient_chunks;
	
	while quotient_idx > 0 {
		quotient_idx -= 1;
		/*
        ...
        let numerator_slice_len = numerator_max_idx;
     	let numerator_start_idx = quotient_idx + divisor_slice_len - 1;
     	if numerator_start_idx >= numerator_slice_len {
            continue;
		}
		...
		{
			// divide the uppermost bits of the remaining numerator to get "sub_quotient" which will be our guess
			// for this quotient element
			let numerator_hi = if numerator_slice_len - numerator_start_idx > 1 { 
				numerator_chunks[numerator_start_idx + 1]
			} else { 
				0 
			};
			let numerator_lo = numerator_chunks[numerator_start_idx];
			let mut sub_quotient = 
				divide_128_by_64_preshifted_reduced(numerator_hi, numerator_lo, reduced_divisor_hi, divisor_hi);
			...
			// sub_quotient is now the correct sub-quotient for this iteration. add it to the full quotient, and
...
mod unit_tests {
	...
	fn test_divide_128_by_64() {
		for divisor in core::u64::MAX..=core::u64::MAX {
			...
			//for numerator in core::u128::MAX - 10..core::u128::MAX {
		        ...
		        let actual_quotient = divide_128_by_64_helper(numerator as u128, divisor);
	...
	fn test_divisor_128(divisor: u128) {
		...
		//assert_eq!(
		//	big_quotient, actual_big, 
		//	"Actual quotient didn't match expected quotient for max/{}", divisor
		//);
		assert_eq!(
